        ...
        <table className="admin-table">
          <thead>
            <tr>
              ...
              <th>
                <SortHeader filters={filters} label="Arbejdsdage" sortKey="days" />
              </th>
              <th>
                <SortHeader filters={filters} label="Tickets" sortKey="tickets" />
              </th>
              <th>
                <SortHeader filters={filters} label="Snit pr. dag (t)" sortKey="avgDay" />
              </th>
              <th>
                <SortHeader filters={filters} label="Snit pr. ticket (t)" sortKey="avgTicket" />
              </th>
            </tr>
          </thead>
          <tbody>
            {rows.length > 0 ? (
              rows.map((row) => {
                const fulfillmentAlert = row.fulfillmentPct < 0.8 || row.fulfillmentPct > 1;
                ...
                return (
                  <tr key={row.mechanicId}>
                    ...
                    <td className={fulfillmentAlert ? "cell--alert" : undefined}>{formatPercent(row.fulfillmentPct)}</td>
                    <td>{row.workdays}</td>
                    <td>{row.tickets}</td>
              ...
              <tr>
                <td colSpan={10}>Ingen summerede data matcher de valgte filtre.</td>
              </tr>
            )}
          </tbody>
        </table>
